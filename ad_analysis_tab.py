...
LOW_Q = 0.70               # 양의 기울기 하위 25% 이상을 오르막으로 간주(완만 포함)
LOW_Q_FALLBACK = 0.60      # 데이터 약할 때 완화
GAP_TOL_DIVISOR = 300      # 빈틈 메움 허용 폭 = ceil(N / 100) 포인트
MIN_RUN_FRAC = 0.06        # 최소 지속 길이 비율(스파이크 방지)

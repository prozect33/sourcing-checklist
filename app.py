...
            st.markdown("---")
            st.markdown("#### 전체 판매")
            total_sales_qty = st.number_input("전체 판매 수량", step=1, key="total_sales_qty", min_value=0)
            total_revenue = st.number_input("전체 매출액", step=1000, key="total_revenue", min_value=0)

            st.markdown("---")
            st.markdown("#### 광고 판매")
            ad_sales_qty = st.number_input("광고 전환 판매 수량", step=1, key="ad_sales_qty", min_value=0)
            ad_revenue = st.number_input("광고 전환 매출액", step=1000, key="ad_revenue", min_value=0)
            ad_cost = st.number_input("광고비", step=1000, key="ad_cost", min_value=0)
...
            daily_profit = 0
            
            if selected_product_name != "상품을 선택해주세요" and product_data and total_sales_qty > 0:
                # 기본 변수 로드
...
                quantity_val = product_data.get("quantity", 1) # 저장된 총 구매 수량
...
                # 💰 총 비용 계산 (요청된 계산 로직)
                
                # 1. 총 수수료 (전체 매출액 * 수수료율/100 * 1.1)
                total_fee = total_revenue * (fee_rate_val / 100) * vat

                # 2. 총 매입 원가 (매입단가 * 전체 판매 수량)
                total_purchase_cost = unit_purchase_cost * total_sales_qty

                # 3. 총 입출고/배송비 (입출고/배송비 * 전체 판매 수량 * 1.1)
                total_inout_shipping = inout_shipping_cost_val * total_sales_qty * vat

                # 4. 총 물류비 (단위 물류비 * 전체 판매 수량)
                total_logistics_cost = unit_logistics * total_sales_qty

                # 5. 총 관세 (단위 관세 * 전체 판매 수량)
                total_customs_duty = unit_customs * total_sales_qty

                # 6. 총 기타 비용 (단위 기타 비용 * 전체 판매 수량)
                total_etc_cost = unit_etc * total_sales_qty

                # 7. 총 광고비 (총 광고비 * 1.1)
                total_ad_cost_with_vat = ad_cost * vat
...
                    st.warning("상품을 선택하고 전체 판매 수량을 1개 이상 입력해야 저장할 수 있습니다.")
                elif not product_data:
                    st.warning("선택된 상품의 상세 정보가 없습니다. 상품 정보 입력 탭에서 먼저 저장해주세요.")
                else:
                    try:
                        data_to_save = {
...
                        }
                        
                        # 동일 날짜/상품에 대한 기존 기록 확인 및 업데이트 또는 삽입
                        existing_record = supabase.table("daily_sales").select("*").eq("date", report_date.isoformat()).eq("product_name", selected_product_name).execute()
                        
                        if existing_record.data:
                            # 업데이트
                            supabase.table("daily_sales").update(data_to_save).eq("date", report_date.isoformat()).eq("product_name", selected_product_name).execute()
                            st.success(f"'{selected_product_name}'의 {report_date} 정산 기록이 업데이트되었습니다.")
                        else:
                            # 삽입
                            supabase.table("daily_sales").insert(data_to_save).execute()
                            st.success(f"'{selected_product_name}'의 {report_date} 정산 기록이 저장되었습니다.")
                    except Exception as e:

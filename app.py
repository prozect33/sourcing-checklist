    ...
    with left:
        ...
        if sell_price_raw.strip():
            try:
                ...
                st.markdown(f"<div style='margin-bottom:15px; font-size:16px; font-weight:bold;'>"
                            f"마진율 {int(target_margin)}% 기준: {format_number(target_cost)}원 ({yuan_cost}위안), "
                            f"마진: {format_number(profit)}원</div>", unsafe_allow_html=True)
            except:
                pass

        ...
        if sell_price_raw.strip():
            try:
                ...
                while left_b <= right_b:
                    ...
                    cost_vat = round(mid * 1.1)
                    fee = round(sell_price_val * float(config['FEE_RATE']) / 100 * 1.1)
                    inout = round(float(config['INOUT_COST']) * 1.1)
                    margin = sell_price_val - (cost_vat + fee + inout)
                    margin_ratio = (margin / supply_price) * 100
                ...
                profit = sell_price_val - (round(target_cost * 1.1) +
                                           round(sell_price_val * float(config['FEE_RATE']) / 100 * 1.1) +
                                           round(float(config['INOUT_COST']) * 1.1))
...
""", unsafe_allow_html=True)
    ...
    with right:
        st.info("계산결과는 아래 계산 버튼 클릭 후 나타납니다.")

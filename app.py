...
def default_config():
    return {
        ...
        "EXCHANGE_RATE": 350,
        "PACKAGING_COST": 500, "GIFT_COST": 1000
    }
def load_config():
...
with tab1:
    ...
    with right:
        if result:
            ...
            # 단위당 반품비용
            unit_costs["return"] = round((unit_costs["pickup"] + unit_costs["restock"]) * (config["RETURN_RATE"]/100))

            # 2) qty 곱해서 “총비용 항목” 생성
            total_costs = {k: v * qty for k, v in unit_costs.items()}
            ...
            for bg, stats in [
                ...
                ("#e3f2fd", [
                    ...
                    ("📉 최소마진율",  f"{(profit/supply_price_total*100):.2f}%"),
                    ("🧾 투자수익률",  f"{roi_total:.2f}%"),
                ])
            ]:
                st.markdown(
                    f"""
...
""", unsafe_allow_html=True)
            ...
            with st.expander("📦 상세 비용 항목 보기", expanded=False):
                ...
                st.write(f"**사은품 비용(총):** {format_number(total_costs['gift'])}원")
                ...
                st.write(f"**최소마진율:** {(profit/supply_price_total*100):.2f}%")

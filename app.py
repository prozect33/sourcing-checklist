...
""", unsafe_allow_html=True)
    ...
    with tab_product:
        ...
        # 4.txt에 존재하는 '기타 비용' 관련 입력 필드 유지 (주석 아님)
    ...
    with tab_daily:
        ...
        # 4.txt에는 이 로직이 없었으므로, 원래의 탭2 UI (st.selectbox)를 그대로 유지합니다.
        ...
        with col6:
            st.markdown(f"**오가닉 판매 수량:** {organic_sales_qty}")
        with col7:
            st.markdown(f"**오가닉 매출액:** {format_number(organic_revenue)}원")
    ...
    with tab_status:
        ...
        # 4.txt에는 이 로직이 없었으므로, 원래의 탭4 UI를 그대로 유지합니다.
        product_list = ["(상품을 선택해주세요)"]
        try:
            response = supabase.table("products").select("product_name").order("product_name").execute()
            if response.data:

...
""", unsafe_allow_html=True)
            ...
            if calc_col.button("계산하기", key="tab1_calc_button"):
                st.session_state["show_result"] = True
            if "show_result" not in st.session_state:
                st.session_state["show_result"] = False
            reset_col.button("리셋", on_click=reset_inputs, key="tab1_reset_button")
    ...
    with tab2:
        ...
        with st.expander("상품 정보 입력"):
            ...
            if st.session_state.is_edit_mode:
                ...
                with col_mod:
                    if st.button("수정하기", key="tab2_modify_button"):
                        if validate_inputs():
                            try:
                                data_to_update = {
                ...
                with col_del:
                    if st.button("삭제하기", key="tab2_delete_button"):
                        try:
                ...
                if st.button("상품 저장하기", key="tab2_save_button"):
                    if validate_inputs():
        ...
        with st.expander("일일 정산", expanded=True):
            ...
            # !!! 실시간 계산 값 표시: 세션 상태의 값을 직접 value로 사용 !!!
            st.number_input(
                "자연 판매 수량",
                value=st.session_state.organic_sales_qty, # <- 세션 상태 값 사용
                disabled=True,
            ...
            st.number_input(
                "자연 판매 매출액",
                value=st.session_state.organic_revenue, # <- 세션 상태 값 사용
                disabled=True,
            ...
            if st.button("일일 정산 저장하기", key="daily_save_button"):
